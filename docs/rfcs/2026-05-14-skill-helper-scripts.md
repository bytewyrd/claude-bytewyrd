...
*Note: A Python YAML parser was considered for P2 (`rfc-frontmatter.sh`) but rejected. `pyyaml` is not in the Python standard library — Python's stdlib has no YAML support at all (the docs.python.org/3/library index lists no `yaml` module; `pyyaml` requires `pip install` or a system package). Python 3 is also not pre-installed on macOS 12.3+ without Homebrew or the Xcode CLT (Apple removed the bundled interpreter in 12.3), making it a less universal dependency than bash. A stdlib-only Python YAML parser would be no simpler than the bounded awk approach, since the awk parser handles the entire schema in ~12 lines. The RFC frontmatter is machine-generated by `rfc-architect` and follows a fixed, deliberately constrained schema (flat `key: "value"` pairs, `~` for null, no multi-line values, no nesting), so the awk parser is fit for purpose. If the schema ever gains complexity that breaks the awk parser, switching to Python + pyyaml is the appropriate follow-up.*
...
markers_json="$(
  grep -n '^\s*[-*>]*\s*FEEDBACK:' "$file" 2>/dev/null \
    | awk -F: '{
        n = $1
...
$ printf '%s\n' \
  ...
  '> FEEDBACK: Blockquote feedback.' \
  > /tmp/test-feedback.md
$ bash scripts/rfc-feedback-list.sh /tmp/test-feedback.md | jq .
{
  "markers": [
    {"line": 3, "text": "FEEDBACK: Add a step for X."},
    {"line": 5, "text": "- FEEDBACK: This needs Y, inside a list item."},
    {"line": 6, "text": "  - FEEDBACK: Nested bullet feedback."},
    {"line": 7, "text": "> FEEDBACK: Blockquote feedback."}
  ]
}
...
| `rfc-feedback-list.sh` | zero markers (0 lines, exit 0), two markers (correct lineno+text), non-`FEEDBACK:` lines not emitted, indented marker (inside list item) detected correctly, nested-bullet marker detected correctly, blockquote marker detected correctly |
...
**Open question: Markdown → JSON for structured RFC extraction.** A Markdown→JSON conversion tool was suggested as a potential alternative for structured RFC content extraction (including `FEEDBACK:` detection); this is out of scope for this RFC but worth tracking as a future option if RFC structure analysis needs grow beyond what grep-based extraction handles. The grep-with-indentation pattern in `rfc-feedback-list.sh` covers the current need (catching markers at any nesting depth in bullets or blockquotes); a Markdown AST parser would only be justified if future skills need to reason about which section a marker belongs to, which list it is in, or other structure-aware queries.
