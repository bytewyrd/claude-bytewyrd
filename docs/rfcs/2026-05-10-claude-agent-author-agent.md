...
The file ships in the plugin's `agents/` directory alongside the existing 45 agents. Any user of the plugin can `@-mention` it, invoke it by name ("use the claude-agent-author agent to create a new..."), or have the main agent delegate to it via the `description` field. The agent file is owned in this repo (no upstream sync since `/agents-update` was removed by RFC `2026-05-10-refactor-command`), so the format expertise propagates to every Bytewyrd project that installs the plugin.
...
**Recommendation: Option A.** This RFC delivers `agents/claude-agent-author.md` and the CLAUDE.md row that documents it. The audit skill is captured as a follow-on dependency: the braindump entry "Audit and rework all agent definitions with claude-agent-author" becomes the next RFC, and it explicitly depends on this one being Done first.
...
These are the names Claude Code recognizes in `tools:` and `disallowedTools:`. Any name not on this list (or not from an installed MCP server) is silently ignored — listing it does not add a tool, but listing it in an otherwise-restrictive `tools:` allowlist means the agent's effective tool set is whatever real names were also listed.
...
- **Never guess from training knowledge.** Any field name, tool name, model alias, or effort level you are not sure about must be verified against the canonical Anthropic docs (`https://code.claude.com/docs/en/sub-agents`, `https://docs.claude.com/en/docs/claude-code/tools-reference`, `https://code.claude.com/docs/en/model-config`) using `mcp__exa__web_fetch_exa`. If the canonical doc disagrees with this prompt's in-prompt reference, the canonical doc wins — and the user should be told the in-prompt reference is out of date so they can update this file.
- **Tool fields are allowlists, not aspirations.** Listing a tool name does not add the tool. Listing a name that is not a real Claude Code tool either does nothing (if the field is otherwise empty) or silently removes that nonexistent entry from the allowlist (if other tools are listed) — either way, the line is at best dead weight and at worst silently restricts the agent to fewer tools than it needs.
...
1. **Frontmatter is minimal and verified.** Only `name`, `description`, `model: opus`, and `color: blue` are set. `tools:` is omitted intentionally — the agent needs `Read`, `Write`, `Edit`, `Grep`, `Glob`, `WebFetch`, `mcp__exa__web_fetch_exa`, and `mcp__exa__web_search_exa` to do its job, and omitting the field inherits all of them. `effort:` is omitted — the session default (`xhigh` on Opus 4.7, `high` on Opus 4.6) is right for this work. `color: blue` matches `rfc-architect`'s color, fitting the "thinks deliberately, writes specs" archetype.
...
Open `/home/divoxx/code/bytewyrd/claude-bytewyrd-workflow/CLAUDE.md`. The current Agent delegation table (as updated by RFC `2026-05-10-refactor-command` once that RFC is implemented) is:
...
(If RFC `2026-05-10-refactor-command` has not yet been implemented at the time this RFC is implemented, the table will not yet contain the "Refactoring (deliberate)" row. Implement against whatever the table is at the time, just inserting the new row in the correct position relative to whatever rows are present.)
