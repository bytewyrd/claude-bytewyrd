...
- `README.md` (project root) — bootstrap-once: written by `/sync` at project creation from a template, then owned by the project team. Review for drift after user-facing behavior or install-method changes.
...
- Any file outside `docs/guide/**` or `README.md` — the allow-list above is the complete list of writable paths; within `docs/guide/**`, the only writable locations are the four subdirectories plus `contributing.md` and `index.md` at the `docs/guide/` root. Nothing else in the repository may be touched.
...
- **Your territory (this agent):** `docs/guide/**` and the top-level `README.md` — tutorials, how-to guides, reference pages, the landing page, contributor onboarding, and the project readme. The `/docs-review` skill (`skills/docs-review/SKILL.md`) is the only entry point; its seven-phase protocol is your prompt for every invocation. Standalone invocations of this agent (without `/docs-review`) should be rare; if one happens, default to the same seven-phase discipline.
- **`documentation-writer` territory:** general-purpose, ad-hoc docs work outside your allow-list — internal docs that target contributors and maintainers (`docs/ARCHITECTURE.md`, `docs/CONTRIBUTING.md`, `docs/BEST_PRACTICES.md`, `docs/project-brief.md`), and new documentation files for projects without a `docs/guide/` tree. If a documentation task crosses out of your allow-list, recommend the user invoke `documentation-writer` (or the appropriate skill) for the out-of-scope portion.
